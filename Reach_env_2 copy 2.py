...
class IRB360Env(gym.Env):
    ...
    def __init__(self, 
        ...
        self.last_time = None
        self.velocity = np.zeros(3)  # 新增速度变量
    ...
    def reset(self, seed=None, options=None):
        ...
        # time.sleep(0.1)
        # self.call_script_function('moveBackToSafePosition')
    ...
    def _get_extended_observation(self):
        ...
        # print(f"Cfunction Velocity: {velocity}")
    ...
    def _calculate_reward(self, current_Pad):
        """改进的奖励计算，结合多目标奖励和新的惩罚机制"""
        block_position = self.current_Block
        
        # 距离奖励
        distance_to_target = np.linalg.norm(np.array(current_Pad) - np.array(block_position))
        distance_reward = 1.0 / (1.0 + distance_to_target)  # 距离越小，奖励越高
        
        # 姿态奖励
        orientation_reward = self._calculate_orientation_reward()  # 姿态越接近目标，奖励越高
        
        # 效率奖励
        efficiency_reward = 1.0 / (1.0 + self.current_step)  # 步数越少，奖励越高
        
        # 速度惩罚
        velocity = self.velocity
        velocity_penalty = -0.1 * np.linalg.norm(velocity)  # 速度越大，惩罚越大
        
        # 逆运动学不稳定惩罚
        ik_penalty = -0.1 if not self.check_ik_stability() else 0.0
        
        # 超出边界惩罚
        boundary_penalty = -0.1 if not self.check_within_boundary(current_Pad) else 0.0
